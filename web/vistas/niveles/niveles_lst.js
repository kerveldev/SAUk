...
function lstRegistros(){
  ...
  nauta.postData(
    'http://sau.test/api/usuarios/nivel/lstNiveles',
    {
      ...
      tabla = nauta.dibujarTabla("regs-tabla",data.data, ["Id"]);
...
function eliminar(_id, _table) {
  ...
  swal({
    ...
    if (willDelete) {// Afirmativo se elimina el registro
      nauta.postData(
        'http://sau.test/api/usuarios/nivel/delNivel',
        {
          "nick"  : dUser.nick,
          "token" : dUser.token,
          "id"    : _id.toString()
