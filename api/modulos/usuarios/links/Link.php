...
function peticion($peticion){
	...
	$tabla['nombre'] = "links";// <<<<<<<<<<<<< Nombre de la tabla en la BD
	...
	// ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
  	switch ($peticion) {
		...
		case 'nvoLink':
			...
			$_d = $x->datos;
			if(empty($_d->Principal)){return $cuerpo = FALTAN_PARAMETROS;}// Si retorna null sale de la peticion
			if(empty($_d->Id_Fuente)){return $cuerpo = FALTAN_PARAMETROS;}// Si retorna null sale de la peticion

			$existe = false;
			if($_d->Principal == "1"){
				$existe = existeModuloPrincipal($_d->Id_Usuario, $_d->Id_Fuente, $_d->Principal);
				if ($existe == false) {
					return peticion_insertar($x->nick, $x->token, USUARIOS['base'], TABLA['nombre'], (array)$_d);
				} else {
					return MODULO_PRINCIPAL_EXISTENTE;
				}
			}else{
				return peticion_insertar($x->nick, $x->token, USUARIOS['base'], TABLA['nombre'], (array)$_d);
			}
        ...
        case 'actLink':
			...
			if($_d->Principal == "1"){
				$existe = existeModuloPrincipal($_d->Id_Usuario, $_d->Id_Fuente, $_d->Principal);
				if ($existe == false) {
					return peticion_actualizar($x->nick, $x->token, USUARIOS['base'], TABLA['nombre'], TABLA['id'], $x->id,(array)$_d);
				} else {
					return MODULO_PRINCIPAL_EXISTENTE;
				}
			}else{
		...
		case 'linksUsuario':
			$fields = array("nick","token", "id", "idfte", "campos", "anexar");// Lista de parametros por recibir
			$box    = new Storer($fields);
			if(empty($x = $box->stocker)){return $cuerpo = FALTAN_PARAMETROS;}// Si retorna null sale de la peticion
			if(!is_array($x->campos)) 	 {return $cuerpo = FALTAN_PARAMETROS;}// Valida que el parametro campos contenga un arreglo
			if(empty($x->id)) 		 {return $cuerpo = ID_VALOR_INVALIDO;}// El 'id' no puede estar vacio

			$where = "WHERE Id_Fuente = ".$x->idfte." AND Id_Usuario = ".$x->id;
					  
			$cuerpo = listarTabla($x->nick, $x->token, $nave, USUARIOS['base'], TABLA, $x->campos, null, $x->anexar, $where);
...
/*
* Verifica si existe un modulo principal para ese Usuario-Fuente-principal
*/
function existeModuloPrincipal($_idUsuario, $_idFuente, $_principal){
	$n = new nauta(USUARIOS['base'], IREK, USUARIOS['ruta']);

	$sql = "SELECT Id_Link FROM links WHERE Principal = $_principal AND Id_Fuente = $_idFuente AND Id_Usuario = $_idUsuario;";
	$r = $n->consultaSQL_asociativo($sql);

	if($r['status'] == true && $r['cant'] > 0){
		return true;
	}else{
		return false;
	}
